...
| Parameter | Typ | Wert | Beschreibung | Quelle |
|---|---|---|---|---|
| `compliance` | geschätzt | 0.80 | Allgemeine Verhaltens-Compliance [0–1]; identisch zur Susceptible-Population. Wirkt ausschliesslich via `adherence` auf die Antibiotikawirkung im Mikro-Layer. | Modellannahme |
| `immune_strength` | Referenzwert | 1.0 | Immunkompetenz-Multiplikator; identisch zur Susceptible-Population (Referenzwert 1.0). Mit der Mikro-Basis-Clearancerate konfundiert → nicht eigenständig identifizierbar. | Modellannahme |
...
| `abx_distribution` | Kontextualisierungsparameter | true | Dosis-Verteilungsmodus: `true` = behandelte Patienten ziehen low/std/high gleichverteilt (gemischte Station, mittlere Wirkung = std); `false` = feste Dosis (`abx_dosage`) für alle Behandelten. Wirkt nur im Mikro-Layer über `DOSE_MULTIPLIERS = {low: 0.6, std: 1.0, high: 1.4}`. | Modellannahme |
| `abx_dosage` | Kontextualisierungsparameter | std | Feste Dosisstufe `low \| std \| high`, nur aktiv wenn `abx_distribution: false`. Erlaubt das Szenario „alle erhalten Hoch-/Niedrigdosis". `high` ≈ +40 % Antibiotikadruck ggü. dem heutigen Schnitt, `low` ≈ −40 %. | Modellannahme |
| `abx_course_length_days` | Kontextualisierungsparameter | 14 | Behandlungskurs in Tagen. `0` = gedächtnislose tägliche Ziehung (altes Verhalten). `>0` = einmal gestartet, bleibt der Patient für so viele Tage auf demselben Regime (echter Kurs, bei Entlassung gekappt). Tägliche Initiierungsrate = `abx_probability / course_length`, damit die Prävalenz erhalten bleibt. Wirkt v. a. bei Carriern, die durch `carrier_extension_days` lange genug bleiben (median LOS ~18 Tage). | Modellannahme |
